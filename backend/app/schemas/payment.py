...
class PaymentCreate(BaseModel):
    ...
    provider: str = Field(..., description="Payment gateway (VNPay, Momo, ZaloPay, etc.)")
    amount: Decimal = Field(..., gt=0, description="Số tiền thanh toán")
    currency_code: str = Field(..., max_length=3, description="Mã tiền tệ (VND, USD, etc.)")
    status: PaymentStatus = Field(default=PaymentStatus.pending, description="Trạng thái payment")
    idempotency_key: str | None = Field(default=None, description="Key để tránh duplicate payment")
...
class PaymentUpdate(BaseModel):
    ...
    currency_code: str | None = Field(default=None, max_length=3, description="Mã tiền tệ")
