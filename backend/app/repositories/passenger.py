...
class PassengerRepository(BaseCRUD[Passenger, PassengerCreate, PassengerUpdate], SearchableRepository[Passenger]):
    def __init__(self, db: AsyncSession):
        BaseCRUD.__init__(self, Passenger, db)
        SearchableRepository.__init__(self, Passenger, db)

    async def get_by_booking(self, booking_id: UUID, *, skip: int = 0, limit: int = 100) -> Sequence[Passenger]:
