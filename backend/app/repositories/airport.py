...
class AirportRepository(BaseCRUD[Airport, AirportCreate, AirportUpdate], SearchableRepository):
    ...
    async def get_by_icao(self, icao: str) -> Airport | None:
        ...
        result = await self.db.exec(
            select(Airport).where(Airport.icao == icao.upper())
        )
        return result.first()

    async def get_by_city_id(self, city_id: UUID, skip: int = 0, limit: int = 100) -> list[Airport]:
