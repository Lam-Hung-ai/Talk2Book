...
class ProviderRepository(BaseCRUD[Provider, ProviderCreate, ProviderUpdate], SearchableRepository):
    ...
    async def get_by_display_name(self, display_name: str) -> Provider | None:
        result = await self.db.exec(select(Provider).where(Provider.display_name == display_name))
        return result.first()
