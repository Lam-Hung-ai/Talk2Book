...
class ReviewRepository(
    BaseCRUD[Review, ReviewCreate, ReviewUpdate], SearchableRepository[Review]
):
    ...
    async def get_by_target(
        self, target_type: str, target_key: str, skip: int = 0, limit: int = 100
    ) -> Sequence[Review]:
    ...
    async def get_by_target_type(
        self, target_type: str, skip: int = 0, limit: int = 100
    ) -> Sequence[Review]:
    ...
    async def get_by_rating(
        self, rating: int, skip: int = 0, limit: int = 100
    ) -> Sequence[Review]:
        """Lấy reviews theo rating"""
        return await self.get_multi(skip=skip, limit=limit, rating=rating)

    async def get_by_rating_range(
        self, min_rating: int, max_rating: int, skip: int = 0, limit: int = 100
    ) -> Sequence[Review]:
        """Lấy reviews trong khoảng rating"""
        statement = (
            select(Review)
            .where(Review.rating >= min_rating, Review.rating <= max_rating)
            .offset(skip)
            .limit(limit)
        )
        result = await self.db.exec(statement)
    ...
    async def get_average_rating_by_target(
        ...
        statement = select(func.avg(Review.rating)).where(
            Review.target_type == target_type, Review.target_key == target_key
        )
        result = await self.db.exec(statement)
        avg = result.one()
    ...
    async def get_recent_reviews(
        self, target_type: str | None = None, limit: int = 10
    ) -> Sequence[Review]:
