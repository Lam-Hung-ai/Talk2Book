...
class RoomRatePlan(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("hotel_id", "name", name="uq_rrp_hotel_name"),
    )
    ...
    cancellation_policy: dict[str, Any] | None = Field(default=None, sa_column=Column(JSONB))

    currency_code: str = Field(foreign_key="currency.code", nullable=False, max_length=3, ondelete="RESTRICT")
    ...
    inventory_items: list["RoomInventoryDaily"] = Relationship(back_populates="rate_plan")
