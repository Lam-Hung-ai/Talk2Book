...
@router.post(
    "/", response_model=PaymentTransactionResponse, status_code=status.HTTP_201_CREATED
)
...
async def search_transactions(
    ...
    return await service.search_transactions(query=q, skip=skip, limit=limit)
...
async def get_payment_transactions(
    ...
    return await service.get_payment_transactions(
        payment_id=payment_id, skip=skip, limit=limit
    )
...
async def get_transactions_by_step(
    ...
    return await service.get_transactions_by_step(step=step, skip=skip, limit=limit)
