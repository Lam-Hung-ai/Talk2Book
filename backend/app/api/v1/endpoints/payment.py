...
def get_payment_service(db: AsyncSession = Depends(get_async_session)) -> PaymentService:
...
async def get_user_payments(
    user_id: UUID,
    status_filter: Optional[str] = Query(None, alias="status", description="Filter theo status"),
    skip: int = Query(0, ge=0),
...
async def update_payment_status(
    payment_id: int,
    new_status: str = Query(..., description="Trạng thái mới: pending, completed, failed, refunded, cancelled"),
    service: PaymentService = Depends(get_payment_service)
):
